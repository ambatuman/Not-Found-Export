...
if uploaded_file is not None:
    ...
    for block in message_blocks:
        ...
        # Saring hanya pesan yang relevan dengan unsur temuan / penyelesaian barang hilang
        if "not found" in block_lower or "missing" in block_lower or "found" in block_lower:
            ...
            if is_vertical_format:
                ...
                # Jika chat vertikal tidak punya nomor PN yang valid, langsung skip (bukan data barang)
                if not has_pn_vertical or pn_val == "-":
                    continue

                # Ambil catatan kaki / kalimat penyelesaian di bawah form vertikal
                for line in lines:
    ...
    if parsed_data:
        ...
        # Tameng filter surplus/minus murni harian
        def filter_strict_pembelaan(row):
            ...
            if any(trash in rem for trash in ["SURPLUS", "MINUS", "WRONG BINNING", "UNRECORDED"]):
                if any(save in rem for save in ["FOUND", "TRANSFER", "ISSUED", "MISSING", "NOT FOUND"]):
                    return True
                return False
            return True
        ...
        # Rekonsiliasi duplikat chat update, ambil yang paling terakhir (keep last)
        df = df_filtered.drop_duplicates(subset=["BIN", "PN", "SN"], keep="last").reset_index(drop=True)
        
        st.success(f"🎉 Sempurna! Chat obrolan koordinasi berhasil dibuang. Data final steril: {len(df)} baris.")
        ...
        st.download_button(
            ...
            file_name="rekap_pembelaan_no_bocor.xlsx",
